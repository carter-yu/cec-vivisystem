...
3. Default pytest uses `FakeLlmParser` (no network, no `XAI_API_KEY`). Live Mini uses SpaceXAI (`XAI_API_KEY`, `https://api.x.ai/v1`, model from `LLM_MODEL`, default `grok-4.5`).
